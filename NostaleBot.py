...
class Bot():
    ...
    def __init__(self, base):
        ...
        self.GAME_HASH = md5(self.HASH_NOSTALE_CLIENTX.encode()+self.HASH_NOSTALE_CLIENT.encode()).hexdigest().upper()
        pass
    ...
    def ConnectToLoginServer(self):
        if self.Connect(self.LOGIN_SERVER_IP, self.LOGIN_SERVER_PORT) == False:
    ...
    def LoginToGameforge(self):
        ...
        if not api.auth(username=self.LOGIN, password=self.PASSWORD):
            print("[LoginToGameforge] Couldn't auth to gameforge (check login and password)!")
            return False
        ...
        for uid, displayName in accounts:
            print("[LoginToGameforge] GF_ACC_ID:", gf_acc_id_n, "Account name:", displayName)
            gf_acc_id_n += 1
    ...
    def CreateLoginPacket(self):
        REGION_CODE = "4"
        return "".join(["NoS0577 ", self.SESSION_TOKEN, " ", self.INSTALLATION_GUID," 003662BF", " ", REGION_CODE, '\v', self.NOSTALE_VERSION, " 0 ", self.GAME_HASH])
